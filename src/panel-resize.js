...
class DragElement {
  ...
  constructor(dragEle, customDragMove, direction = 'horizontal') {
    ...
    this.dragEle.classList.add(this.isHorizontal ? 'horiz-handle' : 'verti-handle');
    this.dragIconMap = {
...
export default class PanelResize {
  ...
  constructor(parentEle, sizeData, direction = 'horizontal') {
  ...
  init() {
    ...
    this.handlesEle.forEach(handle => {
      const handler = new DragElement(handle, true, this.direction);
